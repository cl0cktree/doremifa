	...
	$(document).ready(function(){
				...
				$.getJSON(json_data, function(data){
					$.each(data, function(I, item){
						if($rowId==item.id){
							...
							// ---- 복수 뱃지 정보를 json에서 가져와 해당 필드에 맞게 재구성
							var badge_index_json;
							var badge_value_json;
							var split_all_index = item.badgeIndex.split(' | '); // json 파일의 해당 키값을 구분자로 구분하여 배열의 형태로 나눔.
							var split_all_value = item.badgeValue.split(' | '); // json 파일의 해당 키값을 구분자로 구분하여 배열의 형태로 나눔.
							
							$("#btn2").removeClass('display_none');
							if((Object.keys(item.badgeIndex).length>0)&&(Object.keys(item.badgeValue).length>0)){ // json 파일의 해당 키값이 비었는지 확인.
								if(split_all_index.length>1){ // json 파일의 해당 키값을 구분자로 구분 된 다수의 값을 가지는지 확인.
									badge_index_json = split_all_index;
									badge_value_json = split_all_value;
									for(i=0;i<split_all_index.length;++i){ // 구분자로 구분 된 배열의 값을 순차적으로 구성.
										...
										$("#srchBBB").val("");
										$('.data_sch_list_all').addClass('display_none');
										$('.data_sch_list_all').children('.all_src_data_option_'+src_all_index).addClass('display_none');
									...
									$("#srchBBB").val("");
									$('.data_sch_list_all').addClass('display_none');
									$('.data_sch_list_all').children('.all_src_data_option_'+badge_index_json).addClass('display_none');
								};
								all_badge_arr();
							};
		...
		$('.all_badge_input').on("change click focusin focusout keydown keyup", function(e){
            ...
            if(this_all_badge.siblings('.all_badge_wrap').find('.btn_badge_type2').length > 9){
				this_all_badge.siblings(".all_badge_wrap").css({'width':'100%', 'height':'33px'});
				$('.popup_filter').removeClass('display_none');
				$(".modal_confirm_3").removeClass('display_none');
            }else{
				this_all_badge.siblings(".all_badge_wrap").css({'width':'fit-content'});
			}
        });
        ...
        fnc_remove2 = function(item){
			...
			if(choice_badge_wrap.find('.btn_badge_type2').length < 10){
				...
				choice_badge_wrap.css({'width':'fit-content'});
			}else{
				choice_badge_wrap.css({'width':'100%', 'height':'33px'});
			}
